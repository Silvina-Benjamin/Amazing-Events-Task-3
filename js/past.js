...
for(let event of data.events){
    ...
    if (eventDate < currentDate) {
      ...
      let busqueda = document.querySelectorAll('input[type=search]')
          contenedorPrincipal.innerHTML = "";
      //   let filtrado = busqueda[0].value.toLowerCase();
          ...
          document.addEventListener('submit', (eb)=>{
              ...
              let filtroBusqueda = data.events.filter (eventobusqueda => {
                  if(eventobusqueda.name.toLowerCase() == palabraBusqueda || eventobusqueda.description.toLowerCase().includes(palabraBusqueda )){
                  return true;
                  } 
              })
